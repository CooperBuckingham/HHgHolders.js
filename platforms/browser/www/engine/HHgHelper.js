var HHg = {
  ...
  doForEach: function (arr, func) {
    if (!arr) {
      HHg.warn("WARNING: passed undefined array to doForEach");
      return;
    }
    for (var i = 0; i < arr.length; i++) {
  ...
  returnPositionProps: function (props) {
    ...
    if (props.end !== undefined) { // TODO: cooper, what does this do?
      HHg.warn("end", props.end);
  ...
  returnTimeProps: function (props) {
    var time = 0;
    if (props === undefined) {
      HHg.warn("Passed undefined time property to an action. We'll default it to 0, but 0 time on an action is frequently the sign of an error, since the action won't fire until the next frame.");
    } else if (!isNaN(props)) {
      time = props;
    } else if (props.time !== undefined) {
      time = props.time;
    } else if (props.t !== undefined) {
      time = props.t;
    } else if (props.seconds !== undefined) {
      time = props.seconds;
    }

    if (time === 0) {
      HHg.warn("Passed possible 0 time property to an action. This is frequently the sign of an error, since the action won't fire until the next frame.", props);
    }
    return time;
  },
  ...
  returnColorProps: function (props) {
    ...
    if (typeof props === "string") {
      ...
      var rgb = HHgColorMap[props];
      if (!rgb) {
        HHg.warn("WARNING: color/props passed as argument is not recognized: ", props);
      }
  ...
  // I don't think this is getting used anywhere, was part of the "repeated sequences take up too much memory" issue
